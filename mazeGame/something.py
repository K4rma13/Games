...
while 1:
	key = stdscr.getkey()
	if key=="a" and not quadrados[p.y][p.x].lef:
		p.x-=1
	elif key=="w" and not quadrados[p.y][p.x].top:
		p.y-=1
	elif key=="d" and not quadrados[p.y][p.x].rig:
		p.x+=1
	elif key=="s" and not quadrados[p.y][p.x].bot:
		p.y+=1	
	for qlinha in quadrados:
		for q in qlinha:
			q.draw()
